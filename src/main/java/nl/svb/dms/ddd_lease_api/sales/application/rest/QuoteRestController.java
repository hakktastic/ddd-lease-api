...
class QuoteRestController {
  ...
  @PostMapping
  ResponseEntity<QuoteResponse> fillOutQuote(@Valid @RequestBody FillOutQuoteRequest request)
  ...
  ResponseEntity<QuoteResponse> calculateInstallment(
      @PathVariable("quoteReference") UUID quoteReference)
      throws QuoteNotFoundException {

    log.debug("received CalculateInstallmentRequest: {}", quoteReference);

    final var installmentCalculatedResponse = QuoteResponse.from(
        quoteService.calculateInstallment(quoteReference));
  ...
  ResponseEntity<QuoteResponse> signQuote(@PathVariable("quoteReference") UUID quoteReference,
    ...
    final var signQuoteResponse = QuoteResponse.from(
        quoteService.signQuote(quoteReference, signature));
